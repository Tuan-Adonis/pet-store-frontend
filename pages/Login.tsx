...
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (success) {
        // Find user to redirect correctly
        const foundUser = users.find(u => u.email === email);
        if (foundUser) {
            if (foundUser.roleId === RoleId.ADMIN) navigate('/admin/dashboard');
            else if (foundUser.roleId === RoleId.STAFF) navigate('/staff/dashboard');
            else navigate('/'); 
        } else {
            // Fallback
            navigate('/');
    ...
    <div className="min-h-screen bg-indigo-50 flex flex-col justify-center items-center p-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          >
            {isLoading ? <Loader className="animate-spin" size={20} /> : 'Đăng nhập'}
          </button>
        </form>
        
        <div className="mt-4 text-center space-y-2">
           <p className="text-sm text-gray-600">
            Chưa có tài khoản? <Link to="/register" className="text-indigo-600 hover:text-indigo-800 font-medium">Đăng ký ngay</Link>
          </p>
        ...
        <div className="mt-8 bg-blue-50 border border-blue-100 rounded-lg p-4">
            ...
            <div className="text-xs text-gray-600 space-y-2">
                <div className="grid grid-cols-[80px_1fr] gap-1 items-center">
                    ...
                    <code className="bg-white px-1 rounded border">admin@test.com / 123</code>
                </div>
                <div className="grid grid-cols-[80px_1fr] gap-1 items-center">
                    <span className="font-bold text-gray-700">Staff:</span>
                    <code className="bg-white px-1 rounded border">staff@test.com / 123</code>
                </div>
                <div className="grid grid-cols-[80px_1fr] gap-1 items-center">
                    <span className="font-bold text-gray-700">Customer:</span>
                    <code className="bg-white px-1 rounded border">customer@test.com / 123</code>
                </div>
            </div>
        </div>
      </div>
