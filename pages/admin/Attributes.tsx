...
import { Breed, Origin } from '../../interfaces';
import { Plus, Trash2, X, Save, Tag } from 'lucide-react';
...
export const AdminAttributes: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'breeds') {
        ...
        addOrigin(newOrigin);
        notify('success', 'Đã thêm nguồn gốc mới');
    }
    ...
    <div className="space-y-6">
        <div className="flex justify-between items-center">
            ...
            <button onClick={openAddModal} className="bg-indigo-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-indigo-700">
                <Plus size={20}/> {activeTab === 'breeds' ? 'Thêm Giống' : 'Thêm Nguồn gốc'}
            </button>
        </div>
        ...
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg w-fit">
            <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'breeds' ? 'bg-white shadow-sm text-indigo-700' : 'text-gray-600 hover:text-gray-900'}`}
            >
                Giống loài (Breeds)
            </button>
             <button
                onClick={() => setActiveTab('origins')}
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'origins' ? 'bg-white shadow-sm text-indigo-700' : 'text-gray-600 hover:text-gray-900'}`}
            >
                Nguồn gốc (Origins)
            </button>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                    <tr>
                        ...
                        {activeTab === 'breeds' && (
                             <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Thuộc Danh mục</th>
                        )}
                    ...
                    {activeTab === 'breeds' ? (
                        breeds.map(b => (
                            <tr key={b.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{categories.find(c => c.code === b.categoryCode)?.name || b.categoryCode}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button onClick={() => handleDeleteBreed(b.id)} className="text-red-600 hover:text-red-900"><Trash2 size={18}/></button>
                                </td>
                            </tr>
                        ))
                    ) : (
                         origins.map(o => (
                            <tr key={o.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button onClick={() => handleDeleteOrigin(o.id)} className="text-red-600 hover:text-red-900"><Trash2 size={18}/></button>
                                </td>
                            </tr>
                        ))
                    )}
             ...
             {((activeTab === 'breeds' && breeds.length === 0) || (activeTab === 'origins' && origins.length === 0)) && (
            ...
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
                    <div className="p-6 border-b flex justify-between items-center">
                        <h3 className="font-bold text-lg">{activeTab === 'breeds' ? 'Thêm Giống mới' : 'Thêm Nguồn gốc'}</h3>
                        <button onClick={() => setIsModalOpen(false)}><X size={24}/></button>
                    </div>
                    <form onSubmit={handleSubmit} className="p-6 space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Tên hiển thị</label>
                            <input required type="text" value={name} onChange={e => setName(e.target.value)} className="mt-1 w-full border rounded p-2" placeholder="Ví dụ: Husky, Việt Nam..."/>
                        </div>
