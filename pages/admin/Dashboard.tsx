...
export const AdminDashboard: React.FC = () => {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((o) => {
      ...
      const matchStaff =
        selectedStaffId === "all" ||
        o.staffId === Number(selectedStaffId) ||
        o.staffId === selectedStaffId;
  ...
  const orderStats = calculateRates(
    filteredOrders,
    filteredOrders.length,
    OrderStatus.COMPLETED,
    OrderStatus.CANCELLED
  );
  const serviceStats = calculateRates(
    filteredServices,
  ...
  const orderPieData = getPieData(
    filteredOrders,
    OrderStatus,
    STATUS_COLORS_ORDER
  );
  const servicePieData = getPieData(
    filteredServices,
    ServiceStatus,
    STATUS_COLORS_SERVICE
  );

  // --- REVENUE CHART DATA (BAR CHARTS) ---
  const getChartData = (
    ordersData: typeof filteredOrders,
    servicesData: typeof filteredServices
  ) => {
    const dataMap = new Map<
      string,
      { name: string; orderRevenue: number; serviceRevenue: number }
    >();
    ...
    const process = (date: Date, type: "order" | "service", amount: number) => {
      ...
      if (!dataMap.has(key))
        dataMap.set(key, { name, orderRevenue: 0, serviceRevenue: 0 });
    ...
    if (result.length === 0)
      return [{ name: "No Data", orderRevenue: 0, serviceRevenue: 0 }];
    return result;
  };
    ...
    <div className="space-y-8">
      ...
      {filteredOrders.length === 0 && filteredServices.length === 0 && (
        <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-200 text-yellow-800 text-center">
          Chưa có dữ liệu thống kê trong khoảng thời gian này. Vui lòng tạo đơn
          hàng hoặc lịch hẹn để xem số liệu.
        </div>
      )}
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
          ...
          <div className="w-full bg-gray-100 h-1.5 mt-4 rounded-full">
            <div
              ...
              style={{ width: `${orderStats.completionRate}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
          ...
          <div className="w-full bg-gray-100 h-1.5 mt-4 rounded-full">
            <div
              ...
              style={{ width: `${orderStats.cancellationRate}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
          ...
          <div className="w-full bg-gray-100 h-1.5 mt-4 rounded-full">
            <div
              ...
              style={{ width: `${serviceStats.completionRate}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
          ...
          <div className="w-full bg-gray-100 h-1.5 mt-4 rounded-full">
            <div
              ...
              style={{ width: `${serviceStats.cancellationRate}%` }}
            ></div>
          </div>
        </div>
      </div>
