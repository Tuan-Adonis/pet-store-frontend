...
import { Clock, Calendar as CalIcon, PawPrint, CreditCard, QrCode, X, MessageSquare, CheckCircle, Loader } from 'lucide-react';
...
export const CustomerServices: React.FC = () => {
  ...
  const handleBookingSubmit = (e: React.FormEvent) => {
    ...
    if (selectedService && date && time && petName && petSpecies && petBreed && petAge) {
       if (paymentMethod === 'QR') {
  ...
  const performBooking = async (method: 'COD' | 'QR') => {
      ...
      await bookAppointment(selectedService, date, time, {
        name: petName,
        species: petSpecies,
        breed: petBreed,
        age: parseInt(petAge)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
      ...
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-gray-900">Dịch vụ của chúng tôi</h2>
                ...
                >
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-lg font-semibold flex items-center gap-2">
                        {service.name}
                        {isInactive && <span className="bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full uppercase">Tạm ngưng</span>}
                    </h3>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm h-fit sticky top-24">
        ...
        {!selectedService ? (
          ...
          <form onSubmit={handleBookingSubmit} className="space-y-4">
            <div>
              <p className="text-sm font-medium text-gray-700 mb-1">Dịch vụ đã chọn:</p>
              <p className="text-indigo-600 font-semibold">
                {activeService?.name} - <span className="text-lg">${activeService?.price}</span>
              </p>
            </div>
            ...
            <div className="border-t border-gray-100 pt-4 mt-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-700">Tên bé</label>
                        <input type="text" required value={petName} onChange={e => setPetName(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md text-sm"/>
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-700">Tuổi (tháng)</label>
                        <input type="number" required min="1" value={petAge} onChange={e => setPetAge(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md text-sm"/>
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-700">Loài</label>
                        <select value={petSpecies} onChange={e => setPetSpecies(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md text-sm">
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-700">Giống loài</label>
                        <input type="text" placeholder="VD: Corgi" required value={petBreed} onChange={e => setPetBreed(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md text-sm"/>
                    </div>
                </div>
            </div>
            ...
            <div className="border-t border-gray-100 pt-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-700">Ngày hẹn</label>
                        <input
                            type="date"
                            required
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-700">Giờ hẹn</label>
                        <input
                            type="time"
                            required
                            value={time}
            ...
            <div className="border-t border-gray-100 pt-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div 
                        className={`border rounded-lg p-3 cursor-pointer flex flex-col items-center justify-center text-center gap-1 transition-all ${paymentMethod === 'COD' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'hover:bg-gray-50'}`}
                    ...
                    <div 
                        className={`border rounded-lg p-3 cursor-pointer flex flex-col items-center justify-center text-center gap-1 transition-all ${paymentMethod === 'QR' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'hover:bg-gray-50'}`}
            ...
            >
              ...
              {user ? (paymentMethod === 'QR' ? 'Thanh toán & Đặt lịch' : 'Xác nhận đặt lịch') : 'Đăng nhập để đặt'}
            </button>
          </form>
        )}
      </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-300">
                ...
                <div className="p-8 flex flex-col items-center">
                     <p className="text-gray-600 mb-4 text-center text-sm">
                        Quét mã để thanh toán dịch vụ <strong>{activeService.name}</strong>
                     </p>
                     ...
                     <div className="text-center mb-6">
                        ...
                        <p className="text-3xl font-bold text-indigo-700">${activeService.price}</p>
                     </div>
