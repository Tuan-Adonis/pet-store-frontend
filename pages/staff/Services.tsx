...
export const StaffServices: React.FC = () => {
  const { user } = useAuth();
  const { users } = useUser();
  const { appointments, loadAppointment, updateAppointmentStatus } =
    useAppointment();
  ...
  const getCustomer = (customerId: string | number) =>
  ...
  const isPendingAppt = (a: Appointment) =>
    a.status === ServiceStatus.PENDING ||
    (a.status === ServiceStatus.REQ_CANCEL && !a.staffId);
  ...
  const getFilteredAppointments = () => {
    switch (activeTab) {
      ...
      case "PROCESSING":
        return searchFilteredAppts.filter(
          (a) => isMyAppt(a) && a.status === ServiceStatus.IN_PROGRESS
        );
      case "COMPLETED":
        return searchFilteredAppts.filter(
          (a) => isMyAppt(a) && a.status === ServiceStatus.COMPLETED
        );
      case "CANCELLED":
        return searchFilteredAppts.filter(
          (a) => isMyAppt(a) && a.status === ServiceStatus.CANCELLED
        );
      default:
        return [];
    }
  ...
  const displayedAppointments = getFilteredAppointments().sort(
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex-1">
                ...
                <p className="text-gray-600 text-sm mt-1 flex items-center gap-2">
                  <Calendar size={14} />{" "}
                  <span className="font-bold text-gray-900">{app.date}</span>{" "}
                  lúc{" "}
              ...
              <div className="bg-gray-50 p-3 rounded-lg border">
                ...
                <p className="text-sm">
                  ...
                  <span className="font-medium">
                    {getCustomerName(selectedAppointment.customerId)}
                  </span>
                </p>
                <p className="text-sm mt-1">
                  SĐT:{" "}
                  <span className="font-medium">
                    {getCustomer(selectedAppointment.customerId)?.phone}
                  </span>
                </p>
                <p className="text-sm mt-1">
                  {t("common.note")}:{" "}
