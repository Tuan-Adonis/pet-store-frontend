...
export const StaffDashboard: React.FC = () => {
  ...
  const myServices = useMemo(() => appointments.filter(a => {
  ...
  // --- METRICS ---
  const stats = {
      orderRevenue: myOrders.filter(o => o.status === OrderStatus.COMPLETED).reduce((acc, o) => acc + o.totalAmount, 0),
      serviceRevenue: myServices.filter(a => a.status === ServiceStatus.COMPLETED).reduce((acc, a) => {
          const s = services.find(srv => srv.id === a.serviceId);
          return acc + (s ? s.price : 0);
      }, 0),
      completedOrders: myOrders.filter(o => o.status === OrderStatus.COMPLETED).length,
      cancelledOrders: myOrders.filter(o => o.status === OrderStatus.CANCELLED).length,
      completedServices: myServices.filter(a => a.status === ServiceStatus.COMPLETED).length,
      cancelledServices: myServices.filter(a => a.status === ServiceStatus.CANCELLED).length,
  };
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
          ...
          <div className="bg-white px-4 py-2 rounded-lg border shadow-sm text-sm font-medium text-gray-600 flex items-center gap-2">
              <Calendar size={16}/> Tháng {today.getMonth() + 1} / {today.getFullYear()}
          </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-gray-900">Doanh thu theo ngày (Dựa trên ngày tạo)</h2>
          ...
          <div className="h-80">
             <ResponsiveContainer width="100%" height="100%">
                 <BarChart data={chartData}>
                    ...
                    <Legend />
                    <Bar dataKey="orderRevenue" name="Doanh thu Đơn" fill="#6366F1" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="serviceRevenue" name="Doanh thu Dịch vụ" fill="#10B981" radius={[4, 4, 0, 0]} />
                 </BarChart>
             </ResponsiveContainer>
          </div>
      </div>
