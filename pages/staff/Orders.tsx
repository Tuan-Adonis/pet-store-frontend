...
import { Eye, X, AlertTriangle, AlertCircle, User, Calendar, MapPin, Clock, ShoppingBag, Search, RefreshCw, Phone } from 'lucide-react';
...
export const StaffOrders: React.FC = () => {
  ...
  const isPendingOrder = (o: Order) => o.status === OrderStatus.PENDING || (o.status === OrderStatus.REQ_CANCEL && !o.staffId);
  ...
  const getFilteredOrders = () => {
    switch (activeTab) {
      ...
      case 'PROCESSING':
        return searchFilteredOrders.filter(o => isMyOrder(o) && (o.status === OrderStatus.ACCEPTED || o.status === OrderStatus.SHIPPING || (o.status === OrderStatus.REQ_CANCEL && o.staffId)));
      case 'RE_DELIVERY':
         return searchFilteredOrders.filter(o => isMyOrder(o) && o.status === OrderStatus.RE_DELIVERY);
      case 'COMPLETED':
        return searchFilteredOrders.filter(o => isMyOrder(o) && o.status === OrderStatus.COMPLETED);
      case 'CANCELLED':
        return searchFilteredOrders.filter(o => isMyOrder(o) && o.status === OrderStatus.CANCELLED);
      default: return [];
    }
  };
  ...
  const getStatusColor = (status: number) => {
    switch(status) {
      ...
      case 6: return 'bg-indigo-100 text-indigo-800 border-indigo-200';
      default: return 'bg-gray-100 text-gray-800';
    }
  };

  const handleReceiveOrder = (orderId: string | number) => {
      updateOrderStatus(orderId, OrderStatus.ACCEPTED, 'Nhân viên đã nhận đơn', user?.id);
      setSelectedOrder(null);
      notify('success', 'Đã tiếp nhận đơn hàng thành công');
  };
  
  const handleProcessOrder = (orderId: string | number, nextStatus: OrderStatus) => {
      let note = '';
      if (nextStatus === OrderStatus.ACCEPTED) note = 'Bắt đầu xử lý đơn hàng';
      if (nextStatus === OrderStatus.SHIPPING) note = 'Đã giao cho đơn vị vận chuyển';
      if (nextStatus === OrderStatus.COMPLETED) note = 'Giao hàng thành công';
      if (nextStatus === OrderStatus.RE_DELIVERY) note = 'Giao hàng thất bại, lên lịch giao lại';
  ...
  const confirmCancel = () => {
      ...
      updateOrderStatus(cancelModal, OrderStatus.CANCELLED, cancelReason, user?.id); 
      setCancelModal(null);
      setSelectedOrder(null);
    ...
    <div className="space-y-6">
       ...
       <div className="flex overflow-x-auto p-2 bg-white rounded-lg border border-gray-200 gap-2">
            ...
            <TabButton 
                ...
                count={searchFilteredOrders.filter(o => isMyOrder(o) && (o.status === OrderStatus.ACCEPTED || o.status === OrderStatus.SHIPPING)).length}
                color="blue"
            />
            <TabButton 
                ...
                count={searchFilteredOrders.filter(o => isMyOrder(o) && o.status === OrderStatus.RE_DELIVERY).length}
                color="indigo"
            />
                <TabButton 
        ...
        <div className="grid grid-cols-1 gap-4">
            {displayedOrders.length === 0 && <EmptyState label="Không có đơn hàng nào" />}
            {displayedOrders.map(order => (
                <div key={order.id} className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm hover:shadow-md transition-all">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-1">
                                ...
                                {order.isLate && <span className="bg-red-100 text-red-600 text-xs font-bold px-2 py-0.5 rounded flex items-center gap-1"><AlertCircle size={12}/> Late</span>}
                            </div>
              ...
              <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="p-6 space-y-4">
                      <div className="bg-gray-50 p-3 rounded-lg border">
                          ...
                          <p className="text-sm">Tên: <span className="font-medium">{getCustomerName(selectedOrder.customerId)}</span></p>
                          <p className="text-sm mt-1">SĐT: <span className="font-medium">{getCustomer(selectedOrder.customerId)?.phone}</span></p>
                          <p className="text-sm mt-1">{t('common.address')}: <span className="font-medium">{selectedOrder.shippingAddress || 'N/A'}</span></p>
                          <p className="text-sm mt-1">{t('common.note')}: <span className="italic">{selectedOrder.note || 'Không có'}</span></p>
                          ...
                          <ul className="space-y-2">
                              {selectedOrder.items.map((item, i) => (
                                  <li key={i} className="flex justify-between text-sm border-b pb-2">
                                      <span>{item.quantity}x {item.product.name} ({item.product.breed})</span>
                      ...
                      <div className="pt-4 border-t space-y-3">
                           ...
                           {selectedOrder.status === OrderStatus.ACCEPTED && selectedOrder.staffId === user?.id && (
                               <>
                                <button 
                                    onClick={() => handleProcessOrder(selectedOrder.id, OrderStatus.SHIPPING)}
                           ...
                           {selectedOrder.status === OrderStatus.SHIPPING && selectedOrder.staffId === user?.id && (
                               <div className="grid grid-cols-2 gap-3">
                                   <button 
                                    onClick={() => handleProcessOrder(selectedOrder.id, OrderStatus.RE_DELIVERY)}
                                   ...
                                   >
                                       <RefreshCw size={16} className="mb-1"/> {t('action.process_redelivery')}
                                   </button>
                                   <button 
                                    onClick={() => handleProcessOrder(selectedOrder.id, OrderStatus.COMPLETED)}
                           ...
                           {selectedOrder.status === OrderStatus.RE_DELIVERY && selectedOrder.staffId === user?.id && (
                               <div className="grid grid-cols-2 gap-3">
                                   <button 
                                    onClick={() => openCancelModal(selectedOrder.id)}
                                    className="w-full border border-red-300 text-red-600 py-3 rounded-lg text-sm hover:bg-red-50 font-bold"
...
const TabButton = ({ active, onClick, label, count, color }: any) => {
    ...
    else if (color === 'indigo') activeClass = 'bg-indigo-100 text-indigo-800 border-indigo-300';
